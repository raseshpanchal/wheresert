...
$query=mysqli_query($link, "SELECT * FROM socialmedia_master WHERE ID='$newID'");
$view=mysqli_fetch_array($query);
$newMediaName=$view['MediaName'];
$newMediaURL=$view['URL'];
...
<script src="app/js/validMediaLink.js"></script>
...
<script>
    $(document).ready(function() {
        $("#btnSave").click(function() {
            ...
            $.post("app/editSocialMediaEntry?ID=<?php echo $newID ?>",
                $("#myForm").serialize(),
                function(data) {
                    //Update Edited Table Row
                    $("#1_<?php echo $newID ?>").css('background-color', '#a3d3d1');
                    $("#3_<?php echo $newID ?>").html(myShow + ' ' + myTitle);
                    closeFormDelete();
                });
            return false;
        });
...
<form name="myForm" id="myForm" method="POST">
    <div class="row" style="padding:15px;">
        <div class="col-xs-12">
            <input type="text" class="form-control form-require" id="txt_title" name="txt_title" value="<?php echo $newMediaURL ?>" required placeholder="Enter URL" />
        </div>
    </div>
    ...
    <div class="row" style="padding:15px">
        <div class="col-xs-12">
            ...
            <button type="submit" class="btn btn-danger" style="padding:6px; width:100px;" id="btnSave" onMouseDown="validMediaLink()">Update</button>
        </div>
    </div>
